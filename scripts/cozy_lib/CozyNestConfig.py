...
with open(Static.DEFAULT_CONFIG_FILENAME, 'r') as f:
    _default_config = json.loads(f.read())
    f.close()
...
class CozyNestConfig:
    ...
    def init(self):
        if self.config['webui'] == CozyNestConfig.WEBUI_UNKNOWN:
            if hasattr(shared, 'get_version'):
